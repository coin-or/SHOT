...
using fmtold::internal::MakeUnsigned;
...
class ReadError : public Error {
 private:
  ...
  void init(fmtold::CStringRef filename, int line, int column,
            fmtold::CStringRef format_str, fmtold::ArgList args);

 public:
  /** Constructs the exception object. */
  ReadError(fmtold::CStringRef filename, int line, int column,
            fmtold::CStringRef format_str, fmtold::ArgList args) {
  ...
  FMTOLD_VARIADIC_(char, , ReadError, init,
                fmtold::CStringRef, int, int, fmtold::CStringRef)
};
...
class BinaryReadError : public Error {
 ...
 public:
  /** Constructs the exception object. */
  BinaryReadError(
      const std::string &filename, std::size_t offset, fmtold::CStringRef message)
...
class NLHandler {
 public:
  ...
  /**
    ...
    \endrst
   */
  void OnFunction(int index, fmtold::StringRef name,
  ...
  /**
    ...
    *kind* specifies the suffix kind.
    \endrst
   */
  IntSuffixHandler OnIntSuffix(fmtold::StringRef name, suf::Kind kind,
                               int num_values) {
  ...
  /**
    ...
    *kind* specifies the suffix kind.
    \endrst
   */
  DblSuffixHandler OnDblSuffix(fmtold::StringRef name, suf::Kind kind,
                               int num_values) {
    internal::Unused(&name, kind, num_values);
  ...
  /**
    ...
    \endrst
   */
  Expr OnString(fmtold::StringRef value) {
    internal::Unused(&value);
...
class ReaderBase {
 ...
 public:
  ReaderBase(NLStringRef data, fmtold::CStringRef name);
...
class TextReader : public ReaderBase {
 private:
  ...
  void DoReportError(
      const char *loc, fmtold::CStringRef format_str,
      const fmtold::ArgList &args = fmtold::ArgList());
 ...
 public:
  TextReader(NLStringRef data, fmtold::CStringRef name);

  Locale &locale() { return locale_; }

  void ReportError(fmtold::CStringRef format_str, const fmtold::ArgList &args) {
...
void NLReader<Reader, Handler>::ReadSuffix(int info) {
  ...
  fmtold::StringRef name = reader_.ReadName();
  reader_.ReadTillEndOfLine();
 ...
 public:
  ...
  void Read(fmtold::CStringRef filename, Handler &handler, int flags) {
    ...
    if (size_ == rounded_size_) {
      ...
      fmtold::internal::MemoryBuffer<char, 1> array;
      Read(array);
      return ReadNLString(
...
class HasBuilder {
 private:
  template <typename U> static fmtold::internal::Yes &test(typename U::Builder *);
  template <typename U> static fmtold::internal::No &test(...);
 public:
  enum {value = sizeof(test<T>(0)) == sizeof(fmtold::internal::Yes)};
};
}  // namespace internal

template <typename Handler>
void ReadNLString(NLStringRef str, Handler &handler,
                  fmtold::CStringRef name, int flags) {
