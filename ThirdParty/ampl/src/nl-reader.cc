...
void mp::ReadError::init(
    fmtold::CStringRef filename, int line, int column, fmtold::CStringRef format_str, fmtold::ArgList args)
{
...
fmtold::Writer& mp::operator<<(fmtold::Writer& w, const NLHeader& h)
{
    ...
    if(h.ampl_options[USE_VBTOL_OPTION] == READ_VBTOL)
        w << ' ' << h.ampl_vbtol;
    w << '\n';
    w.write(" {} {} {} {} {} {}\n", h.num_vars, h.num_algebraic_cons, h.num_objs, h.num_ranges, h.num_eqns,
        h.num_logical_cons);
    w.write(" {} {} {} {} {} {}\n", h.num_nl_cons, h.num_nl_objs, h.num_compl_conds - h.num_nl_compl_conds,
        h.num_nl_compl_conds, h.num_compl_dbl_ineqs, h.num_compl_vars_with_nz_lb);
    w.write(" {} {}\n", h.num_nl_net_cons, h.num_linear_net_cons);
    w.write(" {} {} {}\n", h.num_nl_vars_in_cons, h.num_nl_vars_in_objs, h.num_nl_vars_in_both);
    w.write(
        " {} {} {} {}\n", h.num_linear_net_vars, h.num_funcs, h.format == NLHeader::TEXT ? 0 : h.arith_kind, h.flags);
    w.write(" {} {} {} {} {}\n", h.num_linear_binary_vars, h.num_linear_integer_vars, h.num_nl_integer_vars_in_both,
        h.num_nl_integer_vars_in_cons, h.num_nl_integer_vars_in_objs);
    w.write(" {} {}\n", h.num_con_nonzeros, h.num_obj_nonzeros);
    w.write(" {} {}\n", h.max_con_name_len, h.max_var_name_len);
    w.write(" {} {} {} {} {}\n", h.num_common_exprs_in_both, h.num_common_exprs_in_cons, h.num_common_exprs_in_objs,
        h.num_common_exprs_in_single_cons, h.num_common_exprs_in_single_objs);
    return w;
}

mp::internal::ReaderBase::ReaderBase(NLStringRef data, fmtold::CStringRef name)
    : ptr_(data.c_str()), start_(ptr_), end_(ptr_ + data.size()), token_(ptr_), name_(name.c_str())
{
}

template <typename Locale>
mp::internal::TextReader<Locale>::TextReader(NLStringRef data, fmtold::CStringRef name)
...
void mp::internal::TextReader<Locale>::DoReportError(
    const char* loc, fmtold::CStringRef format_str, const fmtold::ArgList& args)
{
    int line = line_;
...
template <typename Locale> bool mp::internal::TextReader<Locale>::ReadOptionalDouble(double& value)
{
    SkipSpace();
    if(*ptr_ == '\n')
        return false;
...
template <typename Locale> fmtold::StringRef mp::internal::TextReader<Locale>::ReadString()
{
    int length = ReadUInt();
    if(*ptr_ != ':')
    ...
    ++line_;
    line_start_ = ++ptr_;
    return fmtold::StringRef(length != 0 ? start : 0, length);
}

template <typename Locale> fmtold::StringRef mp::internal::TextReader<Locale>::ReadName()
{
    SkipSpace();
    const char* start = ptr_;
    if(*ptr_ == '\n' || !*ptr_)
    ...
    return fmtold::StringRef(start, ptr_ - start);
}

template <typename Locale> void mp::internal::TextReader<Locale>::ReadHeader(NLHeader& header)
{
    ...
    header.num_eqns = -1;
    if(ReadOptionalUInt(header.num_ranges) && ReadOptionalUInt(header.num_eqns))
    {
    ...
    bool all_compl = ReadOptionalUInt(header.num_compl_conds) && ReadOptionalUInt(header.num_nl_compl_conds)
        && ReadOptionalUInt(header.num_compl_dbl_ineqs) && ReadOptionalUInt(header.num_compl_vars_with_nz_lb);
    ...
    if(header.num_nl_vars_in_both >= 0)
    { // ampl versions >= 19930630
...
void mp::internal::BinaryReaderBase::ReportError(fmtold::CStringRef format_str, const fmtold::ArgList& args)
{
    fmtold::MemoryWriter w;
...
template <typename File> void mp::internal::NLFileReader<File>::Open(fmtold::CStringRef filename)
{
    file_ = File(filename, fmtold::File::RDONLY);
    ...
    std::size_t page_size = fmtold::getpagesize();
    std::size_t remainder = size_ % page_size;
    rounded_size_ = remainder != 0 ? (size_ + page_size - remainder) : size_;
}

template <typename File> void mp::internal::NLFileReader<File>::Read(fmtold::internal::MemoryBuffer<char, 1>& array)
{
