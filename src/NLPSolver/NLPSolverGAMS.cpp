...
static
void GEV_CALLCONV gevwritecallback(const char* msg, int mode, void* usrmem)
{
   ...
   // puts(msg+1);
   cbdata->env->output->outputInfo(msg+1);
...
E_NLPSolutionStatus NLPSolverGAMS::solveProblemInstance()
{
    ...
    gevwritecallback_data cbdata = {
       ...
       // if run from within GAMS, then EntryPointsGAMS will have installed an spdlog sink, so that messages to env->output go through modelingEnvironment
       // since there is only one modelingEnvironment, we need to take extra care that output send to gevwritecallback() gets to the original modelingEnvironment output stream
