...
void GAMS2OS::createOSObjects()
{
   ...
   if( gmoN(gmo) > 0 && !osinstance->setVariables(gmoN(gmo), varnames, varlow, varup, vartypes) )
	   throw std::logic_error("OSInstance::setVariables did not succeed.");
   ...
   else if( gmoObjNLNZ(gmo) > 0 )
   {
	   ...
	   if( nl == NULL )
	   {
		   sprintf(buffer, "Failure when processing GAMS instructions for objective %s.", osinstance->getObjectiveNames()[0].c_str());
		   throw std::logic_error(buffer);
	   }
   ...
   for( int i = 0; i < gmoM(gmo); ++i )
   {
	   ...
	   else if( gmoGetEquOrderOne(gmo, i) == gmoorder_NL )
	   {
		   ...
		   if( nl == NULL )
		   {
			   sprintf(buffer, "Failure when processing GAMS instructions for equation %s.", osinstance->getConstraintNames()[i].c_str());
			   throw std::logic_error(buffer);
		   }
...
OSnLNode* GAMS2OS::parseGamsInstructions(
