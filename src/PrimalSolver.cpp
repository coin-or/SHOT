...
bool PrimalSolver::checkPrimalSolutionPoint(PrimalSolution primalSol)
{
    ...
    if (env->problem->properties.numberOfQuadraticConstraints > 0)
    {
        ...
        {
            auto tmpLine = boost::format("       Quadratic constraints are not fulfilled. Most deviating %3%: %2% > %1%.") %
    ...
    if (env->problem->properties.numberOfNonlinearConstraints > 0)
    {
        ...
        {
            auto tmpLine = boost::format("       Nonlinear constraints are not fulfilled. Most deviating %3%: %2% > %1%.") %
